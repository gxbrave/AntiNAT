...
Linux amd64 is the primary target. Local functional tests, race tests, static checks, browser checks, and isolated installer tests are covered, but this candidate is `SUPPORTED_WITH_LIMITS`, not a production release.

- Linux amd64 is the most complete target, with systemd as the primary installer path.
- Windows amd64 is mainly covered by builds and limited installer checks.
- Native arm64/OpenRC, real public WAN and CPE/router evidence, production signing, registry OCI digests, and long-running soak evidence are still missing.
...
sudo env ANTINAT_ROLE=both \
  ANTINAT_RELEASE_BASE_URL="https://github.com/gxbrave/AntiNAT/releases/download/<version>" \
  bash scripts/install.sh install \
  --controller-endpoint https://your-controller.example
```

For networks that need a GitHub mirror, use `ghfast.top` as the release URL prefix:

```bash
sudo env ANTINAT_ROLE=both \
  ANTINAT_RELEASE_BASE_URL="https://ghfast.top/https://github.com/gxbrave/AntiNAT/releases/download/<version>" \
  bash scripts/install.sh install \
  --controller-endpoint https://your-controller.example
```

`ANTINAT_RELEASE_BASE_URL` is the right setting for this URL-prefix mirror. `--github-proxy` is for a real HTTP(S) proxy server and is a different option. Use `ANTINAT_ROLE=controller` for only the Controller or `ANTINAT_ROLE=agent` for only the Agent. The release installer currently targets Linux amd64; see [`docs/installer-contract.md`](docs/installer-contract.md).
