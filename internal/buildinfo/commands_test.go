...
func TestVersionCommandsReportLinkerInjectedMetadata(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "antinat-controller", path: "./cmd/antinat-controller"},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			ldflags := fmt.Sprintf("-X github.com/gxbrave/AntiNAT/internal/buildinfo.Version=%s -X github.com/gxbrave/AntiNAT/internal/buildinfo.Commit=%s -X github.com/gxbrave/AntiNAT/internal/buildinfo.Date=%s", version, commit, date)
